...
describe("skill context parity stability diagnostics", () => {
  ...
  it("strictly rejects malformed Phase 5F results while accepting valid unavailable results", async () => {
    ...
    const unavailable = parityResult(snapshot(), snapshot(), {
      success: false, state: "failed", comparisonAvailable: false, consistent: false, mismatchCodes: [],
      reasonCodes: ["direct_trigger_failure"], direct: null, runtime: snapshot(),
      directTriggerAttempted: true, directTriggerSucceeded: false, directResultParsed: false,
      runtimeTriggerAttempted: true, runtimeTriggerSucceeded: true, runtimeResultParsed: true,
    });
