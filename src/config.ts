...
export function loadSkillConfig(): SkillConfig {
  ...
  const feedbackDiagnosticsEnabled = enabled && parseBooleanEnv(
  ...
  const lifecycleReviewEnabled = enabled && parseBooleanEnv(
    env["AGENTMEMORY_SKILL_LIFECYCLE_REVIEW"],
    false,
  );
  ...
  return {
    enabled,
    feedbackEnabled,
    feedbackDiagnosticsEnabled,
    feedbackDiagnosticsLimit: parseClampedInt(
      env["AGENTMEMORY_SKILL_FEEDBACK_DIAGNOSTICS_LIMIT"],
      100,
      1,
      500,
    ),
    feedbackReducerEnabled,
    lifecycleReviewEnabled,
