...
import type {
  ...
  SkillContextParityDriftSignatureFamily,
  SkillContextParityDriftSignatureTransitionClass,
  SkillContextParityDriftSignatureTransitionDiagnosticsInput,
  SkillContextParityDriftSignatureTransitionDiagnosticsReasonCode,
  SkillContextParityDriftSignatureTransitionDiagnosticsResult,
  SkillRecallInput,
} from "../types.js";
...
type ParsedSample = { signature: SkillContextParityDriftSignature; family: SkillContextParityDriftSignatureFamily } | { unavailable: true };
...
export function evaluateSkillContextParityDriftSignatureTransition(input: {
  firstSignature: SkillContextParityDriftSignature;
  secondSignature: SkillContextParityDriftSignature;
}): { transitionClass: SkillContextParityDriftSignatureTransitionClass; signatureChanged: boolean; familyChanged: boolean } {
  const firstFamily = familyFor(input.firstSignature);
  const secondFamily = familyFor(input.secondSignature);
...
function result(
  success: boolean,
  enabled: boolean,
  state: SkillContextParityDriftSignatureTransitionDiagnosticsResult["state"],
  reasonCodes: SkillContextParityDriftSignatureTransitionDiagnosticsReasonCode[],
  values: Partial<SkillContextParityDriftSignatureTransitionDiagnosticsResult> = {},
): SkillContextParityDriftSignatureTransitionDiagnosticsResult {
  return {
    success,
    enabled,
...
function normalizeInput(value: unknown): { recall: SkillRecallInput; request: Omit<RequestInput, "project" | "agentId"> } | null {
  ...
  const input = value as SkillContextParityDriftSignatureTransitionDiagnosticsInput;
...
function parse(value: unknown): ParsedSample | null {
  ...
  if (sample.state === "stable_consistent" && exact(sample.reasonCodes, ["stable_consistency_signed"]) && signature === stableSignature) return { signature, family: "stable_consistent" };
  if (sample.state === "stable_mismatch" && exact(sample.reasonCodes, ["stable_mismatch_signed"]) && mismatchSignatures.has(signature)) return { signature, family: "stable_mismatch" };
  if (sample.state === "observed_drift" && exact(sample.reasonCodes, ["observed_drift_signed"]) && observedSignatures.has(signature)) return { signature, family: "observed_drift" };
  return null;
}

export function registerSkillContextParityDriftSignatureTransitionDiagnosticsFunction(sdk: ISdk): void {
  sdk.registerFunction("mem::skill-context-parity-drift-signature-transition-diagnostics", async (data: unknown) => {
    if (!loadSkillConfig().contextEnabled) {
      return result(true, false, "disabled", ["context_disabled"], { reason: "skill context parity drift signature transition diagnostics is disabled" });
    }
    const input = normalizeInput(data);
    if (!input) return result(false, true, "failed", ["invalid_input"], { reason: "invalid skill context parity drift signature transition diagnostics input" });
    const request = () => buildSkillContextParityDriftSignatureTransitionRequest({ project: input.recall.project!, ...(input.recall.agentId ? { agentId: input.recall.agentId } : {}), ...input.request });
    const unavailableReason = "skill context parity drift signature transition diagnostics could not classify two signature samples";

    let firstRaw: unknown;
    try { firstRaw = await sdk.trigger(request()); } catch {
      return result(false, true, "failed", ["first_signature_trigger_failure"], { reason: unavailableReason, firstSignatureTriggerAttempted: true });
    }
    ...
    if (!first) return result(false, true, "failed", ["invalid_first_signature_result"], { reason: unavailableReason, ...firstFlags });
    if ("unavailable" in first) return result(false, true, "failed", ["first_signature_classification_unavailable"], { reason: unavailableReason, ...firstFlags });

    let secondRaw: unknown;
    try { secondRaw = await sdk.trigger(request()); } catch {
      return result(false, true, "failed", ["second_signature_trigger_failure"], { reason: unavailableReason, ...firstFlags, secondSignatureTriggerAttempted: true });
    }
    ...
    if (!second) return result(false, true, "failed", ["invalid_second_signature_result"], { reason: unavailableReason, ...flags });
    if ("unavailable" in second) return result(false, true, "failed", ["second_signature_classification_unavailable"], { reason: unavailableReason, ...flags });
    const evaluation = evaluateSkillContextParityDriftSignatureTransition({ firstSignature: first.signature, secondSignature: second.signature });
    return result(true, true, evaluation.signatureChanged ? "signature_transition" : "signature_unchanged", [evaluation.signatureChanged ? "signature_transition_observed" : "signature_unchanged"], { ...flags, transitionAvailable: true, ...evaluation });
  });
}
