...
| `memory_decision_audit` |  | `mode`: string, `action`: string, `sourceFunction`: string, `insertionPoint`: string, `project`: string, `agentId`: string, `sessionId`: string, `limit`: number | Inspect Decision Engine audit records without mutating memory state. |
| `memory_decision_candidates` |  | `kind`: string, `status`: string, `project`: string, `agentId`: string, `sessionId`: string, `decisionId`: string, `candidateId`: string, `limit`: number | Inspect Decision Engine candidate queue rows without mutating memory state. |
...
| `memory_skill_feedback_diagnostics` |  | `skillId`*: string, `skillVersion`: number, `kind`: string, `attribution`: string, `project`: string, `agentId`: string, `sessionId`: string, `limit`: number | Inspect append-only AgentSkill feedback evidence without mutating memory state or changing skill lifecycle. |
| `memory_skill_promotion_eligibility` |  | `proceduralMemoryId`*: string | Explain whether one ProceduralMemory meets the read-only AgentSkill promotion policy without creating a skill. |
| `memory_skill_promotion_inventory` |  | `policyEligible`: boolean, `currentlyPromotable`: boolean, `alreadyPromoted`: boolean, `promotionStateResolved`: boolean, `reasonCode`: string, `scanLimit`: number, `limit`: number | Inspect read-only ProceduralMemory promotion eligibility and source-lineage resolution without creating skills. |
| `memory_skill_recall` |  | `project`: string, `agentId`: string, `query`: string, `files`: array, `concepts`: array, `limit`: number | Read active AgentSkill advisories without mutating memory state or injecting context. |
| `memory_skills` |  | `status`: string, `project`: string, `agentId`: string, `concept`: string, `file`: string, `limit`: number | Inspect future AgentSkill diagnostics without mutating memory state. |
